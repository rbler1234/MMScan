...
class LeoEmbodiedScanL(LeoBase):
    ...
    def __init__(self, cfg, split):
        ...
        self.MMScan_loader = MMScan(version='v1', split=split, verbose=True, ratio = 1.0 if split == 'train' else 0.1,task= "MMScan-QA")
    ...
    def parse_dict(self,data_dict):
        ...
        if obj_id is not None and len(obj_id)>0 and scan_pcds.shape[0]>0:
            ...
            for input_bbox in input_bboxes:
                ...
                orientation=np.array(euler_to_matrix_np(bbox[np.newaxis,6:])[0])
