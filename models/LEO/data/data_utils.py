...
def transform_point_cloud(points, translation, orientation):
    ...
    rotated_points = np.dot(points, rotation_matrix.T)  # Transpose the rotation matrix for right multiplication
...
def vis_scene_qa(
        save_path, scene_path, question,
        answer_gt, answer_candidate,
    ...
    if situation is not None:
        ...
        quaternion = [
            rotation["_x"].item(), rotation["_y"].item(), rotation["_z"].item(), rotation["_w"].item()
        ]
