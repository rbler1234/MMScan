...
class ScannetReferenceDataset(Dataset):
    ...
    def parse_dict(self, data_dict)->dict:
        ...
        parse_data_dict = {}
        parse_data_dict["point_clouds"] = point_cloud.astype(np.float32) # point cloud data including features
        parse_data_dict["lang_feat"] = lang_feat.astype(np.float32) # language feature vectors
        parse_data_dict["lang_len"] = np.array(lang_len).astype(np.int64) # length of each description
        parse_data_dict["center_label"] = target_bboxes.astype(np.float32)[:,0:3] # (MAX_NUM_OBJ, 3) for GT box center XYZ
        parse_data_dict["heading_class_label"] = angle_classes.astype(np.int64) # (MAX_NUM_OBJ,) with int values in 0,...,NUM_HEADING_BIN-1
        parse_data_dict["heading_residual_label"] = angle_residuals.astype(np.float32) # (MAX_NUM_OBJ,)
        parse_data_dict["target_bbox"] = target_bboxes.astype(np.float32)
        parse_data_dict["target_rot_mat"] = target_bboxes_rot_mat.astype(np.float32)
        parse_data_dict["size_class_label"] = size_classes.astype(np.int64) # (MAX_NUM_OBJ,) with int values in 0,...,NUM_SIZE_CLUSTER
        parse_data_dict["size_residual_label"] = size_residuals.astype(np.float32) # (MAX_NUM_OBJ, 3)
        parse_data_dict["num_bbox"] = np.array(num_bbox).astype(np.int64)
        parse_data_dict["sem_cls_label"] = target_bboxes_semcls.astype(np.int64) # (MAX_NUM_OBJ,) semantic class index
        parse_data_dict["box_label_mask"] = target_bboxes_mask.astype(np.float32) # (MAX_NUM_OBJ) as 0/1 with 1 indicating a unique box
        ...
        parse_data_dict["ref_box_label"] = ref_box_label.astype(bool) # 0/1 reference labels for each object bbox
