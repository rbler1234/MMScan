...
from mmscan_tool import MMScan
...
class Dataset(ScanNetBaseDataset):
    ...
    def _get_scan_data(self, ori_pcds, data_bboxes, input_bbox =None):
        ...
        instance_bboxes = [instance_bbox[:6] for instance_bbox in instance_bboxes]
        ...
        if input_bbox is not None:
            ...
            ret_dict["input_box_centers_normalized"] =np.zeros((MAX_NUM_OBJ,3)).astype(
                np.float32
            )
            ret_dict["input_box_sizes_normalized"] = np.zeros((MAX_NUM_OBJ,3)).astype(np.float32)
