...
class MMScanDataLoader(embodiedScan.EmbodiedScan):
    ...
    def __init__(self,args,
        ...
        version='v1', split='train', verbose=True):
        
        super(self,MMScanDataLoader).__init__(version=version, split=split_set, verbose=verbose)
    ...
    def _get_scan_data(self, ori_pcds, data_bboxes, input_bbox =None):
        ...
        instance_bboxes = [np.array(data_bboxes[obj_id]["bbox"]) for obj_id in data_bboxes]
        instance_bboxes = [self.down_9DOF_to_6DOF(pcd_data,instance_bbox) for instance_bbox in instance_bboxes]
        ...
        if not self.use_color:
            ...
            point_cloud[:, 3:] = (point_cloud[:, 3:]*256.0 - MEAN_COLOR_RGB) / 256.0
        ...
        if input_bbox is not None:
            ...
            input_bbox = np.array([self.MMScan_loader.down_9DOF_to_6DOF(pcd_data,_bbox) for _bbox in input_bbox])
        ...
        if input_bbox is not None:
            embodied_scan_box_corners = self.dataset_config.box_parametrization_to_corners_np(
                embodied_scan_box_centers[None, ...],
                embodied_scan_raw_sizes.astype(np.float32)[None, ...],
                np.zeros((embodied_scan_box_centers.shape[0],), dtype=np.float32)[None, ...],
            )
        ...
        ret_dict = {}
        if input_bbox is not None:
            ret_dict["input_box_corners"] = embodied_scan_box_corners.astype(np.float32)
            ret_dict["input_box_centers"] = embodied_scan_box_centers.astype(np.float32)
            ret_dict["input_box_centers_normalized"] =embodied_scan_box_centers_normalized.astype(
                np.float32
            )
            ret_dict["input_box_sizes_normalized"] = embodied_scan_box_sizes_normalized.astype(np.float32)
        else:
            ret_dict["input_box_corners"] = np.zeros((MAX_NUM_OBJ,24)).astype(np.float32)
            ret_dict["input_box_centers"] = np.zeros((MAX_NUM_OBJ,3)).astype(np.float32)
            ret_dict["input_centers_normalized"] =np.zeros((MAX_NUM_OBJ,3)).astype(
                np.float32
            )
            ret_dict["input_sizes_normalized"] = np.zeros((MAX_NUM_OBJ,3)).astype(np.float32)
    ...
    def parse_dict(self,data_dict)->dict:
        ...
        if data_dict["input_bboxes"] is not None:
            assert len(data_dict["input_bboxes"]) == len(data_dict["input_bboxes_id"]) 
            for bbox in data_dict["input_bboxes"]:
                input_bboxes.append(torch.tensor(bbox))
            # make it coordinated
            input_bboxes = input_bboxes[:self.max_prompts]
            ret_dict = self._get_scan_data(data_dict['ori_pcds'],data_dict['bboxes'], input_bbox = input_bboxes)
            boxes = self._encode_box_coords(ret_dict)
        else:
            ret_dict = self._get_scan_data(data_dict['ori_pcds'],data_dict['bboxes'], input_bbox = None)
            boxes = None
        ...
        if self.split == 'train':
            caption = self.data_dict['answers'][0]
        ...
        if data_dict['input_bboxes'] is not None:
            if random.random() > 0.5:
                ...
                for _index in range(len(input_bboxes)):
                    box_query[_index] = ret_dict["input_box_corners"][_index].reshape(8, 3).astype(np.float32)
                    box_mask[_index] = 1
            else:
                # use click to identify an object
                for _index in range(len(input_bboxes)):
                    click_query[_index] = ret_dict["input_box_centers"][_index].reshape(3,).astype(np.float32)
                    click_mask[_index] = 1
        else:
            box_query = np.zeros((self.max_prompts, 8, 3))
            box_mask = np.zeros((self.max_prompts,))
        ...
        ret_dict['gradient_mask'] = (llm_inputs['attention_mask'][0] - prompt_inputs['attention_mask'][0]).astype(np.float32)
...
class Dataset(ScanNetBaseDataset):
    ...
    def __init__(
        ...
        self.MMScan_loader = MMScanDataLoader(args,
                                                dataset_config,
                                                split_set,
                                                num_points,
                                                use_color,
                                                ...
                                                use_height,
                                                augment,
                                                version='v1', split=split_set, verbose=True)
        self.MMScan_loader.set_lang_task("MMScan-QA")
        

        # only need this for convenient evaluation
        self.eval_func = evaluate
        self.annotations = MMScan_loader.MMScan_collect["anno"]
