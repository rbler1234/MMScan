...
class EulerInstance3DBoxes(BaseInstance3DBoxes):
    ...
    def overlaps(cls, boxes1, boxes2, mode='iou', eps=1e-4):
        ...
        import json
        with open("./show_error.json",'w') as f:
            json.dump(str(corners1.shape) + str(corners2.shape) + str(eps),f)
        with open("./show_error2.json",'w') as f:
            json.dump(str(corners1) + str(corners2),f)
