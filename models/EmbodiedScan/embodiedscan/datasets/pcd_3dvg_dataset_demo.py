...
def build_demo_info(demo_scene_id='office'):
    data_dir = os.path.join('data/open_scan', 'demo_scene_id')
    ...
    for i in range(1, n_frames):
        ...
        image_ann = dict(img_path=os.path.join('demo', demo_scene_id, 'rgb',
                                               timestamp + '.jpg'),
                         depth_path=os.path.join('demo', demo_scene_id, 'depth',
                                                 timestamp + '.png'),
...
class PointCloud3DGroundingDatasetDemo(BaseDataset):
    ...
    def load_language_data(self):
        ...
        for anno in mmengine.track_iter_progress(language_annotations):
            ...
            data = self.scans.get(language_info['scan_id'], {})
            language_info['scan_id'] = anno['scan_id']

            ann_info = data.get('ann_info', {})

            # save the bounding boxes and corresponding labels
            language_anno_info = dict()
            language_anno_info['is_view_dep'] = self._is_view_dep(
                language_info['text'])
            labels = ann_info.get('gt_labels_3d', np.zeros(0))  # all box labels in the scan
            bboxes = ann_info.get('gt_bboxes_3d', np.zeros((0, 9)))  # BaseInstanceBboxes
