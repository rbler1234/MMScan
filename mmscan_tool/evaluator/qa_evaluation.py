import torch
from mmscan_tool.evaluator.metrics.lang_metric import (
    EM_evaluation,
    coco_evaluate,
    sbert_evaluator,
    simcse_evaluator,
...
class QA_Evaluator:
    ...
    def update(self, batch_input):
        ...
        for _input in batch_input:
            ...
            _input["gt"] = [
                special_token_filter(
                    i, clean=True, truncation=True, max_length=self.max_length
                )
                for i in _input["gt"]
            ]
    ...
    def start_evaluation(self):
        ...
        for metric in self.eval_metric:
            if metric not in coco_scores.keys():
                eval_dict.update(
                    {metric: sum(store_dict[metric]) / len(store_dict[metric])}
                )
            else:
