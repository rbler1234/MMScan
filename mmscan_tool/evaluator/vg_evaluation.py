...
from mmscan_tool.utils.box_utils import euler_iou3d_bbox, euler_to_matrix_np
from mmscan_tool.evaluator.metrics.box_metric import (
...
class VG_Evaluator:
    ...
    def print_result(self):
        ...
        for iou_thr in self.iou_thresholds:
            show_in_table = (
                ["AP", "AR"] + [f"gTop-{k}" for k in self.top_k_visible]
                if not self.call_for_category_mode
                else ["AP", "AR", "AP_C", "AR_C"]
    ...
    def __calculate_iou_array_(self, data_item):
        ...
        iou_matrix = euler_iou3d_bbox(
            pred_center, pred_size, pred_rot, gt_center, gt_size, gt_rot
        )
        ...
        for _index in tqdm(range(len(raw_input))):
            ...
            for mode in ["pred_bboxes", "gt_bboxes"]:
                ...
                raw_input[_index][mode] = {
                    "center": torch.tensor(np.array(center_list)).to(
                        torch.float32
                    ),
                    "size": torch.tensor(np.array(size_list)).to(
                        torch.float32
                    ),
