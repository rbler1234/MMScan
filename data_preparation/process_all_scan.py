...
def create_scene_pcd(es_anno,pcd_result):
    ...
    sorted_indices = sorted(enumerate(bboxes), key=lambda x: -np.prod(x[1][3:6])) 
    ...
    for box, obj_id, obj_type in zip(bboxes, object_ids, object_types):
        ...
        orientation=np.array(euler_angles_to_matrix(
            torch.tensor(box[np.newaxis,6:]),convention='ZXY')[0])
...
def process_one_scan(scan_id,save_root,scannet_root,\
    ...
    try:
        if "scene" in scan_id:
            ...
            pcd_info = create_scene_pcd(dict_1['scannet/'+scan_id],\
                process_scannet(scan_id,scannet_root,scannet_matrix))
            
        elif "mp3d" in scan_id:
            raw_scan_id,region_id = mp3d_mapping[scan_id.split('_region')[0]],\
            ...
            pcd_info = create_scene_pcd(dict_1[mapping_name],\
                process_mp3d(scan_id,mp3d_root,mp3d_matrix,mp3d_mapping))
            
        else:
            if '3rscan/'+scan_id not in dict_1:
                return 
            pcd_info = create_scene_pcd(dict_1['3rscan/'+scan_id],\
...
if __name__ == '__main__':
    ...
    TYPE2INT = np.load(args.train_pkl_path, allow_pickle=True)\
        ["metainfo"]["categories"]
    ...
    trscan_matrix = np.load(f'{args.meta_path}/3rscan_matrix.npy',\
        allow_pickle=True).item()
    with open(f'{args.meta_path}/scans_axis_alignment_matrices.json', 'r') as f:
    ...
    for scan_id in scan_id_list:
        tasks.append((scan_id,args.save_root,scannet_root,\
                    mp3d_root,trscan_root,scannet_matrix,\
                    mp3d_matrix,trscan_matrix,mapping))
    ...
    if parallel:
        mmengine.utils.track_parallel_progress(\
            process_one_scan, tasks, args.nproc)
    else:
