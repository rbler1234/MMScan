...
def create_scene_pcd(es_anno, pcd_result):
    ...
    sorted_indices = sorted(
        enumerate(bboxes), key=lambda x: -np.prod(x[1][3:6])
    )
    ...
    for box, obj_id, obj_type in zip(bboxes, object_ids, object_types):
        ...
        orientation = np.array(
            euler_angles_to_matrix(
                torch.tensor(box[np.newaxis, 6:]), convention="ZXY"
            )[0]
        )
...
if __name__ == "__main__":
    ...
    TYPE2INT = np.load(args.train_pkl_path, allow_pickle=True)["metainfo"][
        "categories"
    ]
    ...
    mp3d_matrix = np.load(
        f"{args.meta_path}/mp3d_matrix.npy", allow_pickle=True
    ).item()
    trscan_matrix = np.load(
        f"{args.meta_path}/3rscan_matrix.npy", allow_pickle=True
    ).item()
    with open(
        f"{args.meta_path}/scans_axis_alignment_matrices.json", "r"
    ) as f:
        scan2axis_align = json.load(f)
    scannet_matrix = {}
    for scan_id in scan2axis_align:
        scannet_matrix[scan_id] = np.array(
            scan2axis_align[scan_id], dtype=np.float32
        ).reshape(4, 4)
