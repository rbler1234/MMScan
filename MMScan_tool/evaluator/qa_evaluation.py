...
from MMScan_tool.evaluator.metrics.lang_metric \
    import EM_evaluation,coco_evaluate,sbert_evaluator,simcse_evaluator
from MMScan_tool.utils.lang_utils import special_token_filter 
...
class QA_Evaluator():
    ...
    def __init__(self,model_config = {},max_length=1024, verbose=True) -> None:
        ...
        self.eval_metric = ["EM","refined_EM","Bleu_1", "Bleu_2", "Bleu_3",\
            "Bleu_4","METEOR","ROUGE_L","CIDEr","SPICE"] + self.special_metric
    ...
    def update(self,batch_input):
        ...
        for _input in batch_input:
            ...
            _input["pred"] = [special_token_filter(_input["pred"][0],clean=True,truncation=True,\
                max_length=self.max_length)]
            _input["gt"] = [special_token_filter(i,clean=True,truncation=True,max_length=self.max_length)\
                for i in _input["gt"]]
    ...
    #     """
    #        1. Here you are expected to add the transform from the original format 
    #        to the expected format.
    #        2. After transformation, ea
           
    #         Args:
    #             raw_input (dict): the raw original input does not conform to the MMScan format.

    #         Returns:
    #             dict : the processed input conform to the MMScan format.
    #     """
    ...
    def start_evaluation(self):
        ...
        if 'simcse' in self.special_metric:
            all_simcse_similarity = self.simcse_evaluator.evaluation(self.save_buffer)
        if 'sbert' in self.special_metric:
            all_sbert_similarity = self.sbert_evaluator.evaluation(self.save_buffer)
    ...
    def __check_format__(self,raw_input):
        ...
        """Check if the input conform with mmscan evaluation format.
            Args:
                The input to be checked, should be a list of dict. Every item with the keys:
                ["index", "ID","question","pred",""gt"]
        ...
        """
        assert isinstance(raw_input,list), "The input of MMScan evaluator should be a list of dict. "
        ...
        for _index in range(len(raw_input)):
            ...
            assert "pred" in raw_input[_index] and isinstance(raw_input[_index]["pred"],list) \
                and len(raw_input[_index]["pred"])==1
            assert "gt" in raw_input[_index] and isinstance(raw_input[_index]["gt"],list) \
                and len(raw_input[_index]["gt"])>=1
            assert "question" in raw_input[_index]
            
    
if __name__ == "__main__":
    model_config = {"simcse":"/mnt/petrelfs/linjingli/mmscan_modelzoo-main/evaluation/pc",\
    "sbert":"/mnt/petrelfs/linjingli/mmscan_modelzoo-main/evaluation/st"}
    test_eval = QA_Evaluator(model_config)
    
    test_eval.reset()
    
    input_batch = [{"ID":"a1","question":"Is there a book?","pred":["Yes, there is."],"gt":["Yes, there is."]},
        { "ID":"a2","question":"Is there a book?","pred":["Yes, there is."],"gt":["Yes, there is."]},
        {"ID":"a3","question":"Is there a book?","pred":["Yes, there is."],"gt":["Yes, there is."]},
        {"ID":"a4","question":"Is there a book?","pred":["Yes, there is."],"gt":["Yes, there is."]}
    ]
    
    test_eval.update(input_batch)
    
    test_eval.start_evaluation()
