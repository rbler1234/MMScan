...
class GPT_Evaluator():
    ...
    """
       ...
       Args:
            ...
            model (str) : The GPT model to use, default we use "gpt-mini-4o".
            verbose (bool) : Whether to print the evaluation results or not.

    """
    def __init__(self, eval_size=-1, API_key="", model='gpt-4o-mini',verbose = False):
    ...
    def QA_evaluation(self, QA_sample_dict, thread_index, tmp_path):
        ...
        for QA_ID in tqdm(QA_sample_dict):
            ...
            GPT_INTPUT = {"Question":QA_sample_dict[QA_ID]["question"],
    ...
    def QA_collection(self, num_threads, tmp_path):
        ...
        for ID in static_result:
            eval_dict[QA_metric_map(ID.split('__')[0])].append(int(static_result[ID]["Correct Number"]\
                )/(int(static_result[ID]["Correct Number"])+int(static_result[ID]["Wrong/Missing Number"])))
            eval_dict['overall'].append(int(static_result[ID]["Correct Number"]\
                )/(int(static_result[ID]["Correct Number"])+int(static_result[ID]["Wrong/Missing Number"])))
    ...
    def load_and_eval(self, raw_batch_input, num_threads=1, tmp_path = './'):
        ...
        for _index in range(num_threads):
            IDs_divide_index.append(QA_IDs[len(QA_IDs)//num_threads*_index:\
                len(QA_IDs)//num_threads*(_index+1)])
        ...
        for thread_index in range(num_threads):
            ...
            thread = threading.Thread(target = self.QA_evaluation, \
