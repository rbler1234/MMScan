...
from MMScan_tool.utils.box_utils import \
    euler_iou3d_bbox,euler_to_matrix_np
from MMScan_tool.evaluator.metrics.box_metric \
    import get_average_precision, get_multi_topk_scores
...
class VG_evalautor():
    ...
    def start_evaluation(self):
        ...
        for data_item in tqdm(self.save_buffer):
            ...
            iou_array = self.__calculate_iou_array_(data_item)
    ...
    def collect_result(self):
        ...
        for data_item in self.records:
            ...
            for metric_name in self.eval_metric:
                assert metric_name in data_item, f"Missing {metric_name} in the results! "
                for metric_name in self.eval_metric:
                    category_results[category][metric_name].append(data_item[metric_name])
                    category_results["overall"][metric_name].append(data_item[metric_name])
        ...
        for category in category_results:
            for metric_name in self.eval_metric:
                category_results[category][metric_name] \
                    = np.mean(category_results[category][metric_name])
    ...
    def print_result(self):
        ...
        self.category_records = {key: self.category_records[key] for key in \
            sorted(self.category_records.keys(),reverse=True)}
        ...
        # some metrics
        for iou_thr in self.iou_thresholds:
            for metric_type in ['AP','AR']+[f'Top-{k}' for k in self.top_k_visible]:
    ...
    def __calculate_iou_array_(self, data_item):
        ...
        iou_matrix = euler_iou3d_bbox(pred_center, pred_size,\
            pred_rot, gt_center, gt_size, gt_rot)
    ...
    def __check_format__(self,raw_input):
        ...
        for _index in tqdm(range(len(raw_input))):
            ...
            for mode in ["pred_bboxes","gt_bboxes"]:
                if 'center' in raw_input[_index][mode]:
                    continue
                center_list = []
                size_list = []
                angle_list = []
                ...
                for box_index in range(len(raw_input[_index][mode])):
                    ...
                    _9_DOF_box_ = raw_input[_index][mode][box_index]\
                                    .cpu().numpy()
                ...
                raw_input[_index][mode] = \
                    ...
                    "rot":
                        torch.tensor(np.array(rot_list)).to(torch.float32)}
